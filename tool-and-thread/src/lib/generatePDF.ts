...
export async function generateReceipt(transaction: Transaction): Promise<Buffer> {
  ...
  const addText = (text: string, x: number, y: number, options: {
    ...
    const textLines = doc.splitTextToSize(text, doc.internal.pageSize.width - 40);
  ...
  const drawCircleLogo = () => {
    ...
    doc.text('T&T', centerX, centerY + 1, { align: 'center' });
  };
  ...
  // Company Header
  const centerX = doc.internal.pageSize.width / 2;
  addText('Tool & Thread', centerX, 60, { 
    fontSize: 24, 
    align: 'center', 
    fontStyle: 'bold',
  ...
  transaction.items.forEach((item) => {
    ...
    if (item.quantity > 1) {
      const itemTotalText = `${item.quantity} × ${formatCurrency(parseFloat(item.price), transaction.currency)}`;
      ...
      addText(formatCurrency(parseFloat(item.price), transaction.currency), rightMargin, currentY, { 
        fontSize: 11,
        align: 'right',
        textColor: colors.slate[800], // text-slate-800
        font: 'sans'
  ...
  addText(formatCurrency(parseFloat(transaction.total), transaction.currency), rightMargin, currentY, { 
    fontSize: 16, 
    fontStyle: 'bold',
